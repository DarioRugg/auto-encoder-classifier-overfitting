...
# Autoencoder
def autoencoder(dims, act='relu', init='glorot_uniform', regularization=None, latent_act = False, output_act = False):
    """
    ...
    for i in range(n_internal_layers):
        h = Dense(dims[i + 1], activation=act, kernel_initializer=init, kernel_regularizer=regularizer, name='encoder_%d' % i)(h)

    # bottle neck layer, features are extracted from here
    h = Dense(dims[-1], activation=l_act, kernel_initializer=init, kernel_regularizer=regularizer, name='encoder_%d_bottle-neck' % (n_internal_layers))(h)

    y = h

    # internal layers in decoder
    for i in range(n_internal_layers, 0, -1):
        y = Dense(dims[i], activation=act, kernel_initializer=init, kernel_regularizer=regularizer, name='decoder_%d' % i)(y)

    # output
    y = Dense(dims[0], activation=o_act, kernel_initializer=init, kernel_regularizer=regularizer, name='decoder_0')(y)
