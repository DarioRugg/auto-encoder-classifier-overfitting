...
class DeepMicrobiome(object):
    def __init__(self, experiment_name, data, seed, global_seed, data_dir, logger):
    ...
    def subsample_train_val_split(self, subsample_factor):
        self.X_train, _, self.y_train, _ = train_test_split(self.X_train, self.y_train, 
                                                            train_size=subsample_factor, 
                                                            random_state=self.seed,
    ...
    def ae(self, dims = [50], epochs= 2000, batch_size=100, verbose=2, loss='mean_squared_error', latent_act=False, output_act=False, act='relu', patience=20, val_rate=0.2, no_trn=False):
        ...
        modelName = os.path.join("checkpoints", self.prefix + self.data + '_seed_{}.h5'.format(self.global_seed))
        ...
        if args.log_per_epoch:
            representations_folder, test_losses_folder = self._get_representation_and_test_losses_folders()
            callbacks.append(adaptation_utils.EncodingCallback(self.encoder, representations_folder, 
        ...
        if args.save_loss:
            ...
            self.logger.save_loss(self.history.history)
        ...
        # log metrics for the fitted model
        self.logger.log_ae_metrics(model= self.autoencoder, train=X_inner_train, test=self.X_test, val=X_inner_test)
    ...
    def cae(self, dims = [32], epochs=2000, batch_size=100, verbose=2, loss='mse', output_act=False, act='relu', patience=25, val_rate=0.2, rf_rate = 0.1, st_rate = 0.25, no_trn=False):
        ...
        # log metrics for the fitted model
        self.logger.log_ae_metrics(model= self.cae, train=X_inner_train, test=self.X_test, val=X_inner_test)
    ...
    def classification(self, hyper_parameters, method='svm', cv=5, scoring='roc_auc', n_jobs=1, cache_size=10000):
        ...
        if method == 'svm':
            clf = GridSearchCV(SVC(probability=True, cache_size=cache_size), hyper_parameters, cv=StratifiedKFold(cv, shuffle=True, random_state=self.seed), scoring=scoring, n_jobs=n_jobs, verbose=100, )
            clf.fit(self.X_train, self.y_train)

        # Random Forest
        if method == 'rf':
            clf = GridSearchCV(RandomForestClassifier(n_jobs=-1, random_state=0), hyper_parameters, cv=StratifiedKFold(cv, shuffle=True, random_state=self.seed), scoring=scoring, n_jobs=n_jobs, verbose=100)
            clf.fit(self.X_train, self.y_train)

        # Multi-layer Perceptron
        if method == 'mlp':
            model = KerasClassifier(build_fn=DNN_models.mlp_model, input_dim=self.X_train.shape[1], verbose=0)
            clf = GridSearchCV(estimator=model, param_grid=hyper_parameters, cv=StratifiedKFold(cv, shuffle=True, random_state=self.seed), scoring=scoring, n_jobs=n_jobs, verbose=100)
        ...
        self.logger.log_clf_metrics(model=clf, x_train=self.X_train, y_train=self.y_train, x_test=self.X_test, y_test=self.y_test)
        ...
        # # the seed for that specific repetition
        # metrics.append(self.seed)
        ...
        # with open(self.data_dir + "results/{}/".format(self.data.split("_")[0]) + self.data + "_result_seed-{}.txt".format(self.global_seed), 'a') as f:
    ...
    def saveLossProgress(self):
        ...
        sns.lineplot(x=range(len(self.history.history['loss'])), y=self.history.history['loss'], label="Train")
        sns.lineplot(x=range(len(self.history.history['val_loss'])), y=self.history.history['val_loss'], label="Validation", color="orange")
        plt.title('Training loss')
        plt.ylabel('Loss')
        plt.xlabel('Epoch')
        plt.legend()
        plt.savefig("results/{}/training_plots/" + figureName + '_seed_{}.png'.format(self.global_seed))
        plt.close()
    ...
    def _get_representation_and_test_losses_folders(self):
        assert args.log_per_epoch
        base_folder = os.path.join("results", self.data.split("_")[0], "per_epoch" if not args.subsample else "subsampled", self.experiment_name)
        representations_folder = os.path.join(base_folder, "representations", "seed_{}".format(self.seed))
        test_losses_folder = os.path.join(base_folder, "results", "ae_test_losses.csv")
        return representations_folder, test_losses_folder
    
def get_logging_folders(name, level, log_per_epoch, subsample):
    base_folder = os.path.join("results", level)
    if log_per_epoch:
        base_folder = os.path.join(base_folder, "per_epoch" if not subsample else "subsampled")
    base_folder = os.path.join(base_folder, name)
    
    ae_results_path = os.path.join(base_folder, "ae_results.csv")
    clf_results_path = os.path.join(base_folder, "clf_results.csv")
    losses_dir = os.path.join(base_folder, "losses")

    return ae_results_path, clf_results_path, losses_dir
...
if __name__ == '__main__':
    ...
    ae_results_path, clf_results_path, losses_dir = get_logging_folders(args.name, args.data.split("_")[0], args.log_per_epoch, args.subsample)
    logger = adaptation_utils.CompleteLogger(ae_results_path, clf_results_path, experiment_number=args.seed,
                                             loss_file=losses_dir if args.save_loss else None)
    ...
    try:
        if args.repeat > 1:
            ...
            for i in range(args.repeat):
                run_exp(random.randint(0, 10000), global_seed=args.seed)
        else:
            run_exp(args.seed, global_seed=args.seed)
